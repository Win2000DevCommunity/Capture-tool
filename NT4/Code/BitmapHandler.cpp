...
#include <iostream>
/*
void BitmapHandler::DisplayBitmap(HDC hdc, RECT* rect, HBITMAP hBitmap, int capturedWidth, int capturedHeight) {
    if (hBitmap) {
        HDC hLocalDC = CreateCompatibleDC(hdc);
        SelectObject(hLocalDC, hBitmap);
        int width = rect->right - rect->left;
        int height = rect->bottom - rect->top;
        StretchBlt(hdc, 0, 0, width, height, hLocalDC, 0, 0, capturedWidth, capturedHeight, SRCCOPY);
        DeleteDC(hLocalDC);
    }
}
*/
